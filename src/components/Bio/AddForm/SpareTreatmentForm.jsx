...
import { 
  ...
  CheckCircle, XCircle, Syringe, HeartPulse, ShieldCheck, CalendarClock
} from 'lucide-react';

import SuccessPopup from '../../global/SuccessPopup';

const TreatmentForm = ({ isOpen, onClose }) => {
      ...
      <div className="bg-[#f8fafc] w-full sm:w-[98vw] max-w-[1600px] sm:rounded-2xl shadow-2xl border-0 sm:border border-white flex flex-col h-full sm:max-h-[96vh] overflow-hidden relative">
        ...
        <div className="bg-white border-b border-slate-200 shrink-0 sticky top-0 z-30 shadow-sm">
          <div className="px-4 lg:px-8 py-4 lg:py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 pr-12 sm:pr-8">
            <div className="flex items-center gap-3 lg:gap-4">
              ...
              <div>
                <h2 className="text-lg lg:text-2xl font-black text-slate-800 tracking-tight leading-tight lg:leading-none">Treatment Entry Record</h2>
          ...
          <div className="px-4 lg:px-8 flex flex-wrap items-center gap-x-2 gap-y-2 pb-3 lg:pb-0 w-full">
            {[
              ...
              { id: 'Indication', label: '2. Indication', icon: <Stethoscope size={14}/> },
              { id: 'Biologic', label: '3. Biologic', icon: <Syringe size={14}/> },
              { id: 'Regimen', label: '4. Regimen & Dosing', icon: <CalendarClock size={14}/> },
              { id: 'Administration', label: '5. Administration', icon: <Activity size={14}/> },
              { id: 'Monitoring', label: '6. Monitoring', icon: <HeartPulse size={14}/> },
              { id: 'Safety', label: '7. Safety & Response', icon: <ShieldCheck size={14}/> },
              { id: 'Docs', label: '8. Documentation', icon: <ClipboardList size={14}/> },
            ].map((step) => (
              <button 
                key={step.id}
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-4 lg:space-y-5 scroll-smooth bg-slate-50/50 relative">
          ...
          <SectionCard title="Patient Details" icon={<User size={20}/>} ref={sectionRefs.Patient}>
            <FieldRow label="Patient" type="select" options={['Select Patient']} />
            <FieldRow label="Encounter ID" type="text" placeholder="Auto-generated ID..." />
            <FieldRow label="Treatment Date" type="date" />
          </SectionCard>

          <SectionCard title="Treatment Indication" icon={<Stethoscope size={20}/>} ref={sectionRefs.Indication}>
            <FieldRow label="Indication" type="radio" options={['Severe Asthma', 'Atopic Dermatitis', 'CRSwNP', 'Rheumatoid Arthritis', 'Other']} />
          ...
          <SectionCard title="Biologic" icon={<Syringe size={20}/>} ref={sectionRefs.Biologic}>
          ...
          <SectionCard title="Regimen & Dosing" icon={<CalendarClock size={20}/>} ref={sectionRefs.Regimen}>
            <FieldRow label="Dose Model" type="radio" options={['Fixed', 'Weight-based', 'Biomarker-based']} />
            <FieldRow label="Dose Amount" type="text" placeholder="Select a drug to see dose amounts" />
          ...
          <SectionCard title="Administration" icon={<Activity size={20}/>} ref={sectionRefs.Administration}>
          ...
          <SectionCard title="Monitoring" icon={<HeartPulse size={20}/>} ref={sectionRefs.Monitoring}>
          ...
          <SectionCard title="Safety & Response" icon={<ShieldCheck size={20}/>} ref={sectionRefs.Safety}>
