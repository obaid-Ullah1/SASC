...
// ✅ Import the new Recipe Builder component
import InjNameRecipe from './InjNameRecipe';
...
const AddInjectionName = ({ isOpen, onClose, onAdd, onUpdate, editingItem }) => {
  ...
  // ✅ State to automatically control the Recipe Builder overlay
  ...
  useEffect(() => {
    if (editingItem) {
      setFormData({
        type: editingItem.allergen || 'Standard', 
        injName: editingItem.injName || '',
        compName: editingItem.compName || 'SA',
  ...
  // ✅ Automatically open the Recipe Modal when 'Average' is selected
  useEffect(() => {
    if (formData.type === 'Average' && !editingItem) {
  ...
  const handleRecipeCancel = () => {
    setShowRecipeModal(false);
    // Reset dropdown so user isn't stuck on "Average" without the modal
    setFormData(prev => ({ ...prev, type: 'Standard' }));
  };

  return (
    <>
      <div className="bg-white border-2 border-sky-200 rounded-lg shadow-lg mb-4 animate-in slide-in-from-top-3 fade-in duration-300 overflow-hidden relative z-10">
        ...
        <form onSubmit={handleSubmit} className="p-5 bg-white/50">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            ...
            <div className="md:col-span-3 flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full h-[40px] border-2 border-slate-200 rounded-lg pl-9 pr-4 text-[13px] font-semibold text-slate-700 outline-none focus:border-[#00A3FF] focus:ring-4 focus:ring-[#00A3FF]/10 transition-all appearance-none bg-white cursor-pointer"
                >
            ...
            <div className="md:col-span-3 flex items-center justify-between h-[40px] pl-2 w-full">
      ...
      {/* ✅ INJECTION RECIPES FULL-SCREEN OVERLAY */}
      {showRecipeModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
            <InjNameRecipe 
              onCancel={handleRecipeCancel}
              onSaveComplete={handleRecipeSave}
            />
          </div>
        </div>
      )}
