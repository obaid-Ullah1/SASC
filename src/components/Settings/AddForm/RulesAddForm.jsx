...
const RulesAddForm = ({ isOpen, onClose, onAdd, initialValue = '', isEditing = false }) => {
  ...
  return (
    <div className="bg-white border-2 border-sky-200 rounded-lg shadow-lg mb-5 animate-in slide-in-from-top-3 fade-in duration-300 overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-6 flex items-center gap-4 bg-white/50">
        <input
          type="text"
          ...
          autoFocus
          className="flex-1 h-[42px] border-2 border-slate-200 rounded-lg px-4 text-[14px] font-semibold text-slate-700 outline-none focus:border-[#00A3FF] focus:ring-4 focus:ring-[#00A3FF]/10 transition-all"
        />
        <button 
          type="submit" 
