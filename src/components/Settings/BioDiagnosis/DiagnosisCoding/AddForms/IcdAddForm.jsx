...
const IcdAddForm = ({ isOpen, onClose, onSave, editingData }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onSave(formData);
    setFormData(defaultFormState); // Reset after save
  };

  // COMMON INPUT CLASSES FROM YOUR THEME
  ...
  return (
    <div className="bg-white border-2 border-sky-200 rounded-lg shadow-lg mb-5 animate-in slide-in-from-top-3 fade-in duration-300 overflow-hidden">
      ...
      {/* HEADER ROW (Exactly matching your gradient and shadow styles) */}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col">
            <h3 className="font-bold text-sky-900 text-[13px] tracking-wide uppercase leading-tight">
              {editingData ? 'Update ICD Code' : 'Create New ICD Code'}
            </h3>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5 bg-white/50">
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex flex-col flex-1 min-w-[200px]">
            ...
            >
              ...
              <option value="GINA (Asthma)">GINA (Asthma)</option>
              <option value="Hanifin & Rajka (AD)">Hanifin & Rajka (AD)</option>
              <option value="EPOS (CRSwNP)">EPOS (CRSwNP)</option>
              <option value="ACR/EULAR (RA)">ACR/EULAR (RA)</option>
            </select>
          </div>
        ...
        <div className="flex justify-end mt-2">
          <button 
            type="submit" 
            className="bg-[#00A3FF] hover:bg-[#008CE6] text-white px-12 h-[42px] rounded-lg text-[13px] font-black uppercase tracking-wider shadow-md transition-all active:scale-95"
          >
