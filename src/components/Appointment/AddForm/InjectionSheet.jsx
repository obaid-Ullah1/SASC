...
const InjectionSheet = ({ isOpen, onClose, patientName = "Valencia Chavez , Blanca", patientNo = "1016", dos = "2025-01-02" }) => {
  ...
  return (
    <>
      <div id="printable-injection-sheet" className="fixed inset-0 z-[99999] flex justify-center items-center bg-slate-900/70 backdrop-blur-sm p-4 sm:p-6">
        <div className="bg-[#f8fafc] w-full max-w-[1500px] h-full max-h-[95vh] rounded-xl shadow-2xl flex flex-col border border-slate-300">
          
          <div className="bg-[#FFB000] text-white px-5 py-3 flex items-center justify-between shrink-0 rounded-t-xl">
          ...
          <div className="bg-white border-b-2 border-slate-200 px-6 py-3 flex items-center gap-10 text-[14px] text-slate-700 shrink-0 relative z-10 shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#f8fafc] flex flex-col gap-6">
            ...
            <div className="flex flex-col border border-slate-300 rounded-xl shadow-md bg-white shrink-0">
              <div className="bg-[#00A3FF] text-white font-black text-[15px] px-5 py-2.5 flex items-center justify-between uppercase rounded-t-lg">
                <div className="flex items-center gap-2">
                  <Syringe size={16} className="text-white" /> 
                  <span>Assessment</span>
                </div>
                <div className="bg-white/20 text-white px-3 py-1 rounded-full text-xs font-bold">{assessments.length} Records</div>
              </div>
              
              <div className="flex flex-col rounded-b-lg overflow-hidden">
                <div className="bg-[#e0f2fe] px-5 py-2.5 flex items-center gap-2 text-[#0369a1] text-[13px] font-medium border-b border-slate-200">
                ...
                <div className="w-full overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-[#f1f5f9] text-slate-600 text-[11px] border-b border-slate-300">
                        <th className="px-2 py-2 font-bold text-center border-r border-slate-300 w-16">Type</th>
                        <th className="px-2 py-2 font-bold text-center border-r border-slate-300 w-20">Time/Min</th>
                        ...
                        <th className="px-2 py-2 font-bold text-center w-10"><Trash2 size={14} className="mx-auto" /></th>
                      </tr>
                    </thead>
                    <tbody>
                      {assessments.length === 0 ? (
                        <tr><td colSpan={SYMPTOMS.length + 3} className="text-center py-6 text-slate-400 font-bold text-sm">No assessments added.</td></tr>
                      ) : (
                        assessments.map((row) => (
                          <tr key={row.id} className="border-b border-slate-200 hover:bg-slate-50">
                            <td className="px-2 py-1.5 text-center font-black text-slate-800 text-[12px] border-r border-slate-200">{row.type}</td>
                            <td className="px-2 py-1.5 text-center border-r border-slate-200">
                              <input type="time" value={row.time} onChange={(e) => updateTime(row.id, e.target.value)} className="w-full text-center outline-none bg-transparent" />
                            </td>
                            {SYMPTOMS.map((symptom, idx) => (
                              <td key={idx} className="px-1 py-1.5 text-center border-r border-slate-200 p-0 m-0">
                                <div 
                                  className="flex items-center justify-center w-full min-h-[26px] cursor-pointer hover:bg-blue-50 transition-colors" 
            ...
            {fetchedInjections && (
              <div className="flex flex-col border border-slate-300 rounded-xl shadow-md bg-white shrink-0">
                <div className="bg-[#00A3FF] text-white font-black text-[15px] px-5 py-2.5 flex items-center justify-between uppercase rounded-t-lg">
                  <div className="flex items-center gap-2"><Syringe size={18} /> Injection Details</div>
                  <button type="button" onClick={hideInjectionData} className="text-white/80 hover:text-white bg-white/10 rounded-full p-1"><X size={18} /></button>
                </div>
                
                <div className="w-full overflow-x-auto p-0">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-[#e0f2fe] text-slate-700 text-[11px] font-bold border-b border-slate-300">
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {fetchedInjections.map((inj) => (
                        <tr key={inj.id} className="hover:bg-slate-50">
                          <td className="px-2 py-1.5 font-bold text-slate-700 border-r border-slate-200 text-center">{inj.shot}</td>
                          <td className="px-2 py-1.5 text-slate-800 border-r border-slate-200 font-medium">{inj.injection}</td>
                          <td className="px-2 py-1.5 font-black text-red-500 border-r border-slate-200 text-center">{inj.ratio}</td>
                          <td className="px-2 py-1.5 font-bold text-blue-600 border-r border-slate-200">{inj.code1}</td>
                          <td className="px-2 py-1.5 font-bold text-[#00A3FF] border-r border-slate-200">{inj.volume}</td>
                          <td className="px-2 py-1.5 text-slate-700 border-r border-slate-200 text-center">{inj.bottle}</td>
                          <td className="px-1 py-1 border-r border-slate-200">
                            <input type="text" value={inj.staff} onChange={(e) => handleInjectionChange(inj.id, 'staff', e.target.value)} className="w-full bg-transparent outline-none px-1" />
                          </td>
                          ...
                          <td className="px-1 py-1 border-r border-slate-200">
                            <input type="text" value={inj.code2} onChange={(e) => handleInjectionChange(inj.id, 'code2', e.target.value)} className="w-12 bg-transparent outline-none px-1" />
                          </td>
                          ...
                          <td className="px-2 py-1.5 text-center border-r border-slate-200 p-0">
                            <div 
                              className="flex items-center justify-center w-full min-h-[24px] cursor-pointer" 
                              onClick={() => handleInjectionChange(inj.id, 'rxn', !inj.rxn)}
                            >
                              {inj.rxn && <Check size={18} strokeWidth={4} className="text-green-500" />}
                            </div>
                          </td>
                          <td className="px-1 py-1">
                            <input type="text" value={inj.remarks} onChange={(e) => handleInjectionChange(inj.id, 'remarks', e.target.value)} className="w-full bg-transparent outline-none px-1" />
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {reactionData && (
              <div className="flex flex-col border border-slate-200 rounded-xl shadow-sm bg-white shrink-0 overflow-hidden">
                <div className="bg-[#00A3FF] text-white font-bold text-[14px] px-5 py-2.5 flex items-center justify-between uppercase">
                  <div className="flex items-center gap-2">
                    <Zap size={16} className="fill-white" /> REACTION
                  </div>
                  <button type="button" onClick={hideReactionData} className="text-white hover:text-white bg-transparent hover:bg-white/20 rounded transition-colors"><X size={20} strokeWidth={2} /></button>
                </div>

                <div className="p-6 flex flex-col gap-6 bg-white mt-1">
                  <div className="grid grid-cols-5 gap-6">
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><ListOrdered size={16} className="text-[#00A3FF]" strokeWidth={2.5} /> No</label>
                      <select value={reactionData.no} onChange={e => updateReactionData('no', e.target.value)} className="border border-slate-300 rounded-md px-3 py-2 text-sm text-slate-800 outline-none focus:border-[#00A3FF] w-full shadow-sm bg-white h-[38px]"><option></option><option>1</option><option>2</option><option>3</option></select>
                    </div>
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><MapPin size={16} className="text-[#00A3FF]" strokeWidth={2.5} /> Loc</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><AlertTriangle size={16} className="text-[#00A3FF]" strokeWidth={2.5} /> Systemic</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><Clock size={16} className="text-[#00A3FF]" strokeWidth={2.5} /> Reaction Time</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><ImageIcon size={16} className="text-[#00A3FF]" strokeWidth={2.5} /> Picture</label>
                  ...
                  <div className="grid grid-cols-5 gap-6">
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><Syringe size={16} className="text-[#00A3FF]" strokeWidth={2.5} /> PT Shot</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><Star size={16} className="text-amber-400 fill-amber-400" strokeWidth={2} /> Score</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><div className="w-3 h-3 rounded-full bg-red-500"></div> Wheal</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><Droplet size={16} className="text-red-500 fill-red-500" strokeWidth={2} /> Flare</label>
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center gap-2 text-slate-500 font-bold text-[13px]"><Clock size={16} className="text-[#00A3FF]" strokeWidth={2.5} /> Obs Time</label>
            ...
            <div className="flex flex-col gap-2 mt-2 shrink-0">
              <div className="flex items-center gap-2 text-amber-500 font-bold text-[13px]">
                <div className="w-3.5 h-3.5 bg-amber-400 rounded-sm"></div> Note / Comments
              </div>
              <textarea rows="3" value={note} onChange={(e) => setNote(e.target.value)} className="w-full border border-slate-300 rounded-xl p-4 outline-none text-[13px] text-slate-800 resize-none bg-white"></textarea>
            </div>

            {/* ✅ NEW: SIGNATURE SECTION (ON SCREEN) */}
            <div className="grid grid-cols-3 gap-10 mt-auto shrink-0 mb-0">
              <div className="flex flex-col items-center gap-1">
                <input type="date" className="border-b border-slate-400 focus:border-[#00A3FF] bg-transparent w-full text-center outline-none text-[14px] font-bold text-slate-800 pb-1" />
                <span className="text-[12px] font-bold text-slate-500 uppercase">Date</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <input type="text" className="border-b border-slate-400 focus:border-[#00A3FF] bg-transparent w-full text-center outline-none text-[14px] font-bold text-slate-800 pb-1" />
                <span className="text-[12px] font-bold text-slate-500 uppercase">Staff Member Name</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <input type="text" className="border-b border-slate-400 focus:border-[#00A3FF] bg-transparent w-full text-center outline-none text-[14px] font-bold text-slate-800 pb-1" />
                <span className="text-[12px] font-bold text-slate-500 uppercase">Signature</span>
              </div>
            </div>

          </div>

          <div className="bg-white border-t border-slate-200 px-6 py-4 flex items-center justify-between shrink-0 relative z-10 rounded-b-xl">
            ...
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setShowPrintModal(true)} className="bg-slate-800 hover:bg-slate-700 text-white font-bold text-[14px] px-8 py-2.5 rounded-lg transition-colors flex items-center gap-2 shadow-md">
      ...
      {showPrintModal && (
        <div className="fixed inset-0 z-[100000] flex justify-center items-center bg-slate-900/60 backdrop-blur-sm print:hidden">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-[400px] flex flex-col gap-2">
            <div className="flex items-center gap-3 mb-2 text-slate-800">
              <Printer size={24} className="text-[#00A3FF]" strokeWidth={2.5} />
              <h3 className="font-black text-[18px] m-0 uppercase">Print Options</h3>
            </div>
            <p className="text-slate-500 text-[13px] font-bold mb-4 leading-relaxed">
              Would you like to print the filled record, or a blank form for manual entry?
            </p>
            <div className="flex flex-col gap-3">
              <button onClick={() => triggerPrint('filled')} className="bg-[#00A3FF] text-white font-bold text-[14px] py-3 rounded-lg hover:bg-[#008CE6] transition-all flex justify-center items-center gap-2 shadow-md shadow-[#00A3FF]/20">
                Print Filled Record
              </button>
              <button onClick={() => triggerPrint('unfilled')} className="bg-white border-2 border-slate-200 text-slate-700 font-bold text-[14px] py-3 rounded-lg hover:bg-slate-50 hover:border-slate-300 transition-all flex justify-center items-center gap-2">
                Print Blank Form
              </button>
              <button onClick={() => setShowPrintModal(false)} className="text-slate-400 font-bold text-[13px] py-2 mt-1 hover:text-slate-600 transition-all">
                Cancel
              </button>
            </div>
          </div>
